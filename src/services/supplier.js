...
class supplier {
  ...
  async create(supplier) {
    ...
    const { companyName, contactName, contactEmail } = supplier

    const { street, city, region, postalCode, country, phone } =
      supplier.address

    const checkEmail = await db(table.suppliers).where({ contactEmail }).first()
    if (checkEmail !== undefined)
      throw new Error("There is already a supplier with that email")
  ...
  async update(id, supplier) {
    ...
    const checkEmail = await db(table.suppliers)
      .where({ contactEmail })
      .whereNot("id", id)
      .first()
    if (checkEmail !== undefined)
