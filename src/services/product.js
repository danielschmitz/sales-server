...
class product {
    ...
    async create(product) {
        ...
        if (checkName !== undefined) throw new Error('There is already a product with that name')
    ...
    */
    async update(id, product) {
        ...
        const { name, description } = product
        const checkName = await db(table.products).where({ name }).whereNot('id', id).first()
