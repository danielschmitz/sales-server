...
class shipper {
    ...
    async create(shipper) {
        ...
        if (checkEmail !== undefined) throw new Error('There is already a shipper with that email')
    ...
    */
    async update(id, shipper) {
        ...
        const checkEmail = await db(table.shippers).where({ contactEmail }).whereNot('id', id).first()
        if (checkEmail !== undefined) throw new Error('There is already a shipper with that email')
